...
def init_trainer(conf, now_time, k_fold=0):
    if conf["env"]["task"] in ["classification", "recognition"]:
    ...
    if conf["env"]["mode"] == "train":
        ...
        if "data_path_folds" in conf["dataloader_train"].keys():
            ...
            for idx in range(k_fold):
                conf["dataloader_train"]["data_path"] = conf["dataloader_train"][
                    "data_path_folds"
                ][idx]
                conf["dataloader_valid"]["data_path"] = conf["dataloader_valid"][
                    "data_path_folds"
                ][idx]
