...
class TrainerBase:
    ...
    def __init__(self, conf, now=None, k_fold=0):
        ...
        self.accelerator = (
            Accelerator()
        )  # `$accelearte config` to set configuration at first
        now_time = now if now is not None else datetime.now().strftime("%Y%m%d %H%M%S")
        self.saved_model_directory = os.path.join(
            self.conf["env"]["saved_model_directory"], now_time
        )
        self.k_fold = k_fold
        ...
        if not self.conf["env"]["debug"]:
            ...
            shutil.copy(
                ...
                os.path.join(
                    self.saved_model_directory,
                    os.path.split(self.conf["config_path"])[-1],
                ),
            )
            ...
            utils.Logger().info(
                f"{utils.Colors.BOLD}Running {self.k_fold}th fold...{utils.Colors.END}"
            )
        else:
            utils.Logger(
                "log.txt",
        ...
        if self.conf["model"]["saved_ckpt"] != "":
            if "imagenet" in self.conf["model"]["saved_ckpt"].lower():
                self.model.module.load_pretrained_imagenet(
                    self.conf["model"]["saved_ckpt"]
                )
                utils.Logger().info(
                    f"{utils.Colors.LIGHT_RED}Model loaded successfully!!! (ImageNet){utils.Colors.END}"
                )
            else:
                if "train_only_head" in self.conf["model"].keys():
                    if self.conf["model"]["train_only_head"] is True:
                        self.model.module.freeze_backbone()
                else:
                    self.model.module.load_state_dict(
                        torch.load(self.conf["model"]["saved_ckpt"], weights_only=True)
                    )
                utils.Logger().info(
                    f"{utils.Colors.LIGHT_RED}Model loaded successfully!!! (Custom){utils.Colors.END}"
                )

        # init dataloader
        self.loader_train = self.init_data_loader(
            conf=self.conf, conf_dataloader=self.conf["dataloader_train"]
        )
        self.loader_valid = self.init_data_loader(
        ...
        # init metrics
        self.metric_train = self.init_metric(
            self.conf["env"]["task"], self.conf["model"]["num_class"]
        )
        self.metric_val = self.init_metric(
            self.conf["env"]["task"], self.conf["model"]["num_class"]
        )
        ...
        self._validate_interval = max(
            1, (len(self.loader_train.Loader) // self.conf["env"]["train_fold"]) + 1
        )
    ...
    def save_model(self, epoch, metric=None, metric_name="metric"):
        if not self.conf["env"]["debug"]:
            ...
            # Create a new state dict with only the desired parameters
            if hasattr(self.model.module, "filter_state_dict"):
                torch.save(self.model.module.filter_state_dict(), file_format)
            else:
                torch.save(self.model.module.state_dict(), file_format)

            utils.Logger().info(
                f"{utils.Colors.LIGHT_RED}model saved -> {file_format}{utils.Colors.END}"
            )
    ...
    def set_scheduler(conf, conf_dataloader, optimizer, data_loader):
        ...
        steps_per_epoch = math.ceil(
            (data_loader.__len__() / conf_dataloader["batch_size"])
        )
        ...
        if conf["scheduler"]["name"] == "WarmupCosine":
            ...
            scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
                optimizer,
                T_max=conf["scheduler"]["cycles"],
                eta_min=conf["scheduler"]["lr_min"],
            )
    ...
    def init_model(conf, device):
        model = getattr(model_implements, conf["model"]["name"])(conf["model"]).to(
            device
        )
    ...
    def init_metric(task_name, num_class):
        if task_name in ["classification", "regression", "landmark", "recognition"]:
