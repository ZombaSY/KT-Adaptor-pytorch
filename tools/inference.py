...
class Inferencer:
    def __init__(self, conf):
        ...
        self.model.module.load_state_dict(torch.load(self.conf['model']['saved_ckpt']), strict=False)
        self.model.eval()
    ...
    def inference_classification(self, epoch):
        ...
        for iteration, data in enumerate(self.loader_valid.Loader):
            with torch.no_grad():
                ...
                output_argmax = torch.argmax(output[result_key], dim=1).detach().cpu().numpy()
        ...
        if self.conf['env']['mode'] == 'valid':
            ...
            df = pd.DataFrame({'fn': self.loader_valid.image_loader.df['input'],
        ...
        for iteration, data in enumerate(self.loader_valid.Loader):
            with torch.no_grad():
                ...
                if self.conf['env']['mode'] == 'valid':
                    ...
                    absolute_errors = torch.abs(output[result_key].view(-1) - target.view(-1))
                    correct_predictions += (absolute_errors <= tolerance_level).sum().item()
                    total_predictions += absolute_errors.size(0)  # Update total predictions count
    ...
    def inference_recognition(self, epoch):
        ...
        with torch.no_grad():
            for iteration, data in enumerate(self.loader_valid.Loader):
                ...
                embeddings_before.extend(torch.nn.functional.adaptive_avg_pool2d(output['latent_before'], 1).squeeze())
                # embeddings_after.extend(output[key_result].squeeze())
        ...
        if len(embeddings_before) > 0:
            all_embeddings_before = torch.stack(embeddings_before, dim=-1).detach().cpu().numpy().T
            np.save('visualize/data/cfp-embeddings-after.npy', all_embeddings_before)
            # all_embeddings_after = torch.stack(embeddings_after, dim=-1).detach().cpu().numpy().T
            # np.save('visualize/data/cfp-embeddings-after.npy', all_embeddings_after)
    ...
    def __post_process_regression(self, x_img, target, output, img_id):
        for idx in range(x_img.shape[0]):
            ...
            for i in range(output.shape[-1]):
                ...
                utils.append_data_stats(self.data_stat, key_name, output[idx][i].detach().cpu().item())
