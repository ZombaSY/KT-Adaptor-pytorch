...
class Logger(logging.Logger):
    def __init__(self, dst, level=logging.INFO):
        ...
        write_formatter = logging.Formatter(
...
def cut_mix(_input, _refer, _input_label=None, _refer_label=None):
    ...
    if _input_label is not None and _refer_label is not None:
        ...
        cutout_mask = cv2.resize(
            cutout_mask, (cw_1, ch_1), interpolation=cv2.INTER_NEAREST
        )
...
def cv2_imread(fns_img, color=cv2.IMREAD_UNCHANGED):
    ...
    if img.shape[-1] == 3 and color in [cv2.IMREAD_UNCHANGED, cv2.IMREAD_COLOR]:
        ...
        )  # convert color space for albumentaitons
...
def draw_image(x_img, output_prob, img_save_dir, img_id, n_class):
    ...
    cv2_imwrite(
        os.path.join(img_save_dir, img_fn) + ".png",
        cv2.cvtColor(x_img, cv2.COLOR_RGB2BGR),
    )
    for i in range(1, n_class):
        cv2_imwrite(
            os.path.join(img_save_dir, img_fn) + f"_map_class_{i}.png", output_grey[i]
        )
...
def get_mixup_sample_rate(y_list):
    ...
    for i in range(data_len):
        ...
        # KDE sample rate
        kd = KernelDensity(kernel="gaussian", bandwidth=1.75).fit(
            data_i
        )  # should be 2D
...
def draw_landmark(img, lmk, save_dir, img_fn, put_index=True):
    ...
    for index in range(lmk.shape[0]):
        ...
        draw_pos = (
            int(lmk[index][0] * tmp_img.shape[0]),
            int(lmk[index][1] * tmp_img.shape[1]),
        )
